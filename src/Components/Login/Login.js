...
const Login = () => {
    ...
    useEffect(()=>{
        console.log(loginConfirmed?.data);
    
            if(loginConfirmed?.data.message === "success"){
                navigate("/")
            }
            if(loginConfirmed?.data.message === "Error"){
            ...
            <div className='w-50 m-auto container text-dark mt-5'>
                ...
                <form onSubmit={formik.handleSubmit} className='p-5 row'>
                    ...
                    <div className="form-group mb-3">
                        ...
                        {loginConfirmed?.data.message === 'Error' && <small className="form-text text-danger text-center">{loginConfirmed.data.error}</small>}
                    </div>
