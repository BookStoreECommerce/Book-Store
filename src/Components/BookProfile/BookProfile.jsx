...
export default function BookProfile() {
  ...
  const { isLoading, bookCategory, specBook } = useSelector(
    ({ books }) => books
  );
      ...
      >
        {isLoading ? (
          ...
          <>
              ...
              <div className="row mt-lg-5 mt-0 justify-content-center">
                ...
                <div className=" col-lg-9 col-md-12 mt-3 mt-md-0">
                  ...
                  <span className={styles.rate}>
                    <Rating rate={specBook?.rating} section="profile" />
                    <span className={`ms-2 ${styles.rateColor}`}>
                      {specBook?.rating} {specBook?.rating ? "out of 5" : ""}
                    </span>
                  </span>
                  <h3 className={styles.category}>
                    {specBook?.category?.name}
                  </h3>
                  <p className={styles.description}>
                    {specBook?.desc.length > 250 ? (
                      <>
                        {showMore
                          ? specBook?.desc
                          : `${specBook?.desc.substring(0, 250)}`}
                        <button
                          className={`btn ${styles.btnSeeMore} ms-2`}
                  ...
                  <div>
                    <p className={styles.textColorDetails}>
                      <span className={styles.textColor}>Publisher:</span>{" "}
                      {specBook?.publisher} ({specBook?.published})
                    </p>
                    <p className={styles.textColorDetails}>
                    ...
                    <p className={styles.textColorDetails}>
                      <span className={styles.textColor}>Language:</span>{" "}
                      {specBook?.lang}
                    </p>
                    <p className={styles.textColorDetails}>
                      <span className={styles.textColor}>Paperback:</span>{" "}
                      {specBook?.pages}
                    </p>
                    <p className={styles.textColorDetails}>
                      <span className={styles.textColor}>ISBN:</span>{" "}
                      {specBook?.ISBN}
                    </p>
                  </div>
            ...
            {loading ? (
              <i
                className={`fas fa-spinner fa-spin ${styles.spinnerColor}`}
              ></i>
            ) : (
